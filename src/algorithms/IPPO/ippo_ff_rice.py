...
from utils import logwrapper_callback, IPPO, RICE
from ..utils import BaseTrainerParams
from algorithms import trainer, trainer_params
from networks import create_ppo_networks, ActorNetworkMultiDiscrete, CriticNetwork
from jaxenv.environments.rice import Rice
...
@trainer_params(env_name=RICE,
                algo_name=IPPO)
class IPPOTrainerParams(BaseTrainerParams):
...
@trainer(algo_name=IPPO,
         env_name=RICE)
def build_trainer(
    env: Rice,
    trainer_params: IPPOTrainerParams = IPPOTrainerParams(),
    load_model: str = None
):
    ...
    critic_params = dict(hidden_layers=[256, 256])
    ...
    # networks
    actors, critics = create_ppo_networks(
        key=network_key,
    ...
    def eval_func(key: chex.PRNGKey, train_state: TrainState):
        def step_env(carry, _):
            ...
            for stage in range(eval_env.STEP_STAGES):
                ...
                action_dist = het_models_forward(train_state.actors, obs_v)
    ...
    def train_func(rng: chex.PRNGKey = rng):
        ...
        def _env_step(runner_state, _):
            ...
            for stage in range(env.STEP_STAGES):
                ...
                jax.debug.print("Shape {}", last_obs["observations"].shape)
                action_dist = jax.vmap(het_models_forward, in_axes=(None, 0))(train_state.actors, last_obs)
                action, log_prob = action_dist.sample_and_log_prob(seed=sample_key)
                
                value = jax.vmap(het_models_forward, in_axes=(None, 0))(train_state.critics, last_obs)
        ...
        def _update_epoch(update_state, _):
            ...
            def __ppo_los_fn(params, trajectory_minibatch, advantages, returns):
                ...
                action_dist = jax.vmap(het_models_forward, in_axes=(None, 0))(params["actors"], observations)
                value = jax.vmap(het_models_forward, in_axes=(None, 0))(params["critics"], observations)
        ...
        def train_step(runner_state, _):
            ...
            # calculate gae
            train_state, env_state, last_obs, rng = runner_state
            last_value = jax.vmap(het_models_forward, in_axes=(None, 0))(train_state.critics, last_obs)
