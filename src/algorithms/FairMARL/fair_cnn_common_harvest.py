...
from jaxenv.environments.cleanup import CleanUp
from utils.const import F_MAPPO
from ..utils import BaseTrainerParams, gini, find_first_zero_per_column
from utils import logwrapper_callback, COMMON_HARVEST
...
class CNN(eqx.Module):
    """CNN encoder using default Equinox initializers."""
    conv1: eqx.nn.Conv2d
    dense1: eqx.nn.Linear
    # activation: str

    def __init__(self, obs_shape: Tuple[int, int, int], activation: str = "relu", *, key: jax.random.PRNGKey):
        # self.activation=activation
        h, w, c = obs_shape
        key1, key2 = jax.random.split(key)

        # Layers are now initialized in one line using defaults
        self.conv1 = eqx.nn.Conv2d(in_channels=c, out_channels=16, kernel_size=3, key=key1, )
        
        flattened_size = (h-2) * (w-2) * 16
        self.dense1 = eqx.nn.Linear(in_features=flattened_size, out_features=16, key=key2)
...
class Actor(eqx.Module):
    """Actor network using default initializers."""
    embedding_net: CNN
    actor_dense: eqx.nn.Linear
    actor_mean: eqx.nn.Linear
    # activation: str

    def __init__(self, obs_shape: Tuple[int, int, int], action_dim: int, activation: str = "relu", *, key: jax.random.PRNGKey):
        key_cnn, key_dense, key_mean = jax.random.split(key, 3)
        
        self.embedding_net = CNN(obs_shape, activation, key=key_cnn)
        self.actor_dense = eqx.nn.Linear(in_features=16, out_features=16, key=key_dense)
        self.actor_mean = eqx.nn.Linear(in_features=16, out_features=action_dim, key=key_mean)
    ...
    def __call__(self, obs: jax.Array) -> distrax.Categorical:
        ...
        x = self.actor_dense(embedding)
        x = jax.nn.tanh(x)
        logits = self.actor_mean(x)
        return distrax.Categorical(logits=logits)

class Critic(eqx.Module):
    """Critic network using default initializers."""
    embedding_net: CNN
    ...
    def __init__(self, obs_shape: Tuple[int, int, int], activation: str = "relu", *, key: jax.random.PRNGKey):
        # self.activation=activation
        key_cnn, key_hidden, key_value = jax.random.split(key, 3)

        self.embedding_net = CNN(obs_shape, activation, key=key_cnn)
        self.hidden = eqx.nn.Linear(in_features=16, out_features=16, key=key_hidden)
        self.value = eqx.nn.Linear(in_features=16, out_features=1, key=key_value)
    ...
    def __call__(self, x: jax.Array) -> jax.Array:
        ...
        hidden_state = self.hidden(embedding)
        hidden_state = jax.nn.tanh(hidden_state)
        value_output = jax.nn.softplus(self.value(hidden_state))
        return jnp.squeeze(value_output, axis=-1)
    
@chex.dataclass(frozen=True)
@trainer_params(env_name=COMMON_HARVEST,
                algo_name=F_MAPPO)
...
def rearrange_batch(last_obs, env, num_envs):
    obs_batch = jnp.transpose(last_obs, (0, 1, 4, 2, 3))
    world_state = jnp.transpose(last_obs, (0,2,3,1,4)).reshape(num_envs, *(env.observation_space()[0]).shape[:-1], -1)
...
def build_trainer(
    ...
    def eval_func(key: chex.PRNGKey, train_state: TrainState):
        def step_env(carry, _):
            ...
            obs_batch = jnp.transpose(obs_v, (0, 3, 1, 2))
            
            rng, step_key, sample_key = jax.random.split(rng, 3)
            action_dist = apply(train_state.actors, obs_batch)
    ...
    def train_func(rng: chex.PRNGKey = rng):
        ...
        def _env_step(runner_state, _):
            ...
            obs_batch, world_state = rearrange_batch(last_obs, env, config.num_envs)
        ...
        def _update_epoch(update_state, _):
            ...
            def __ppo_los_fn(params, trajectory_minibatch, advantages, returns, value_0, count):
                ...
                _advantages = (advantages / (value_0 + 1.0e-5))  @ rewards_factor if config.fair else advantages @ rewards_factor
        ...
        def train_step(runner_state, _):
            ...
            rewards = trajectory_batch.reward.sum(axis=0)
            rewards = rewards.mean(axis=0)
            

            # calculate gae
            train_state, env_state, last_obs, rng = runner_state
            _, world_state = rearrange_batch(last_obs, env, config.num_envs)
            ...
            count = train_state.optimizer_state[1][1].count
            ...
            metric["additional"] = {
                    "apples_consumed" : {
                        f"Agent {agent_id}": rewards[agent_id]//num_agents for agent_id in range(len(rewards))
                        }, 
                    "gini_index" : gini(rewards),
                    "ent_coef" : ent_coef_schedule(count)
                }
            try:
                hit_zero = find_first_zero_per_column(metric["remained_apples"], length=config.num_steps).mean()
                metric["additional"]["hit_zero_apples"] = hit_zero
            except Exception:
                pass
