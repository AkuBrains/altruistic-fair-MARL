    ...
    def train_func(rng: chex.PRNGKey = rng):
        ...
        def _update_epoch(update_state, _):
            ...
            def __ppo_los_fn(params, trajectory_minibatch, advantages, returns, value_0, count):
                ...
                _advantages = (advantages / (value_0 + 1))  @ rewards_factor if config.fair else advantages @ rewards_factor
        ...
        def train_step(runner_state, _):
            ...
            metric["additional"] = {
                    "Rewards" : {
                        f"Agent {agent_id}": rewards[agent_id] for agent_id in range(len(rewards))
                        }, 
