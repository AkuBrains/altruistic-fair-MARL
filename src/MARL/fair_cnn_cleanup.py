...
from utils import logwrapper_callback, CLEANUP
...
@trainer_params(env_name=CLEANUP,
                algo_name=F_MAPPO)
class TrainerParams(BaseTrainerParams):
    algo: Literal['mappo', "maa2c"] = "maa2c"
    ...
    def __post_init__(self):
        if self.algo in ["maa2c"]:
            object.__setattr__(self, "num_minibatches", 1)
            object.__setattr__(self, "update_epochs", 1)
        self.num_iterations = int(self.total_timesteps // self.num_steps // self.num_envs)
        self.minibatch_size = int(self.num_envs * self.num_steps // self.num_minibatches)
